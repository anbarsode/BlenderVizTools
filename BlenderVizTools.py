...
def create_slices(arr, sepscale='linear', clim=None, cscale='linear', label='array'):
    ...
    # slice locations
    if sepscale == 'linear': z_locs = np.linspace(0, 1, arr.shape[2])
    elif sepscale == 'log': z_locs = np.geomspace(0, 1, arr.shape[2])
    
    return label, path_to_slices, slice_paths, z_locs

def volume_render(slices_data, kind='cuboid', cmap='my_cold', extent=[[0,1],[0,1],[0,1]], out='script.py', N_layers=None, length_scale=1):
    __LABEL__, path_to_slices, __IMAGE_FILENAMES__, __Z_LOCS__ = slices_data
    __IMAGE_FILENAMES__ = repr(__IMAGE_FILENAMES__)
    __Z_LOCS__ = 'np.' + repr(__Z_LOCS__)
    __COLOR_MAP__ = cmap
    
    __LENGTH_SCALE__ = length_scale
    extent = np.array(extent) / __LENGTH_SCALE__
    
    if kind == 'cuboid' or kind == 'cube':
        __MATERIAL_FUNC__ = 'create_scalar_Cuboid_layer'
        obj_scale = [e[1]-e[0] for e in extent]
        obj_size = 1
        obj_loc = [0.5 * (e[1] - e[0]) - 0.5 for e in extent]
    ...
    param_dir['__LENGTH_SCALE__'] = '%f' % __LENGTH_SCALE__
    param_dir['__N_LAYERS__'] = '%d' % (N_layers if N_layers is not None else len(__Z_LOCS__))
